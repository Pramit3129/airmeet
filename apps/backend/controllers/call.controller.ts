...
import { publishMessage, scheduleTask } from "@airmeet/pubsub";
...
export class CallController {
  ...
  static async scheduleCall(req: Request, res: Response) {
    ...
    // const job = await queue.add("scheduled-call", jobData, {
    //   delay: delayMs,
    //   removeOnComplete: true,
    // });
    const job = await scheduleTask(jobData, delayMs);
    console.log(
      `Job created for ${phNo} and job created job: ${JSON.stringify(job)} with a delay of ${delayMs} mili secs`,
    );
    return res.status(200).json({
  ...
  static async createBatchCall(req: Request, res: Response) {
    ...
    if (newLeadsToInsert.length > 0) {
      try {
        ...
        const deletedCachedData = await leadCache.deleteUserLeads(
          user._id.toString(),
        );
