...
import { scheduleTask } from "@airmeet/pubsub";
    ...
    try {
      ...
      if (event === "call_started") {
        if (isBatchCallRecord) {
          ...
          console.log(
            "Call Record created for batch call record for lead " +
              call?.to_number,
          );
      ...
      try {
        if (needScheduling) {
          try {
            ...
            const job = await scheduleTask(jobData, delayMs);
            console.log(
              "Job added successfully as it is asked for scheduling jobId",
            );
            return res.status(200).json({
        ...
        if (!callRecord) {
          console.log(
            `Call record not found for ${call_id}, creating new one.`,
          );
          callRecord = new Call({
            callId: call_id,
            status: call?.call_status,
