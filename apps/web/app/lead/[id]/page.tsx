...
export default function LeadDetailsPage() {
    ...
    const fetchCalls = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log("Fetched calls data:", data.calls.data);
          if (data.calls.data && data.calls.data.length > 0) {
            const firstCall = data.calls.data[0];
            console.log("First call analysis:", firstCall.analysis);
            if (typeof firstCall.analysis === "object") {
              console.log(
                "Custom Analysis Keys:",
                Object.keys(
                  (firstCall.analysis as any).custom_analysis_data || {},
                ),
              );
            }
          }
  ...
  const sentimentData = calls.reduce(
    (acc, call) => {
      if (typeof call.analysis === "object" && call.analysis !== null) {
        const sentiment =
          (call.analysis as CallAnalysis).user_sentiment || "Unknown";
    ...
    return calls.reduce(
      (acc, call) => {
        if (typeof call.analysis === "object" && call.analysis !== null) {
          ...
          const value =
            (analysis.custom_analysis_data?.[fieldName] as string) || "Unknown";
                    ...
                    {sentimentData.length > 0 && (
                      <Card>
                        <CardHeader>
                          ...
                          <CardDescription>
                            Breakdown of calls by user sentiment
                          </CardDescription>
                        </CardHeader>
                        <CardContent>
                          <ChartContainer
                    ...
                    {(leadIntentData.length > 0 ||
                      ...
                      (() => {
                        ...
                        const urgencyRadialData = urgencyData.map((item) => ({
                          ...
                          count: item.count,
                          fill:
                            getUrgencyScore(item.name) >= 80
                              ? "#EF4444"
                              : getUrgencyScore(item.name) >= 50
                                ? "#F59E0B"
                                : "#22C55E",
                        }));
                        const avgUrgency =
                          urgencyData.length > 0
                            ? Math.round(
                                urgencyRadialData.reduce(
                                  (sum, d) =>
                                    sum + getUrgencyScore(d.name) * d.count,
                                  0,
                                ) / totalCalls,
                              )
                            : 0;
                                    ...
                                    return (
                                      ...
                                      >
                                        <div className="flex items-center gap-1.5">
                                          <div
                                            className={`w-2 h-2 rounded-full ${isPositive ? "bg-[#22C55E]" : "bg-[#6A4CFF]"}`}
                                          />
                                  ...
                                  {nextActionData
                                    ...
                                    .map((item, i) => {
                                      ...
                                      return (
                                        <div
                                          key={i}
                                          className={`p-1.5 rounded text-xs ${i === 0 ? "bg-[#8B5CF6]/10" : "bg-muted/50"}`}
                                        >
                                          <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-1">
                                              <span
                                                className={`w-3.5 h-3.5 rounded-full flex items-center justify-center text-[9px] font-bold ${i === 0 ? "bg-[#8B5CF6] text-white" : "bg-muted-foreground/20"}`}
                                              >
                                                {i + 1}
                                              </span>
